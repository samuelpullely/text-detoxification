...
def run_self_diagnosis_experiment(wrapper: ModelWrapper, examples: List[ModelOutput], attribute_name: str, pattern: str,
    ...
    for example_batch in example_iterator:
        ...
        token_probability_distribution = wrapper.get_token_probability_distribution(input_texts, output_choices=output_choices, pattern=pattern)
...
if __name__ == '__main__':
    ...
    for model_idx, model_name in enumerate(args.models):
        ...
        for attribute in args.attributes:
            ...
            filename = f"{model_name.replace('/', '-')}-{args.output_filename}"
            output_path = os.path.join(args.output_dir, filename)
            with open(output_path, 'a', encoding='utf8') as fh:
